...
The presentation registry is `src/parallel_o2/data/presentation.json`; `questions[].variants` is the complete old-to-new route manifest. Scientific IDs retain their meaning. Python validates the UI-v2 envelope and migrates v1 to map presentation. Active settings own the current experiment; drafts store inactive modes. Fractions remain canonical while saturation editors and labels use percent.

Verification is tracked in `acceptance.json` and the final receipt. Historical receipts qualify their own exact builds and are not UI-refactor evidence. No deployment is authorized by this refactor's delivery plan: the existing Pages workflow remains manual. The hosted application remains the preceding qualified release until separately deployed.

## Independent pending gates

- Clinician comprehension review: **pending**, not inferred from screenshots or automated tests.
